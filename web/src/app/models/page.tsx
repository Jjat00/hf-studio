  ...
  return (
    <div className="px-4 py-8 md:px-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="headline text-[40px] md:text-[56px]">Every model. One studio.</h1>
          <p className="mt-2 text-fg-3">
            {models.length} Higgsfield endpoints with full input schemas{synced && ` · catalog synced ${synced}`}
          </p>
        </div>
        <label className="flex h-11 w-full max-w-xs items-center gap-2 rounded-xl border border-line bg-surface-2 px-3">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search…" className="flex-1 bg-transparent outline-none" />
        </label>
      </div>
            ...
            <div key={m.id} className="flex flex-col rounded-[22px] border border-line bg-surface-2 p-5">
              <div className="flex items-start justify-between gap-2">
                <span className="flex items-center gap-1.5 rounded-xl bg-surface-4 px-2.5 py-1 text-sm font-medium text-fg-2">
                  <Icon className="size-4" /> {m.output === "image" ? "Image" : "Video"}
                </span>
                <a href={m.docs_url} target="_blank" rel="noreferrer" className="text-fg-3 hover:text-fg" aria-label="Docs">
                  <ExternalLink className="size-4" />
                </a>
              </div>
              <p className="mt-4 text-[19px] font-semibold">{name}</p>
              <p className="text-[15px] text-fg-3">{workflow}</p>
