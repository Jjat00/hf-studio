...
export function EmptyState({ empty, art, notes }: { empty: Empty; art: string; notes?: string[] }) {
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col items-center px-6 py-14">
      <h2 className="headline max-w-4xl text-center text-4xl md:text-[56px]">{empty.title}</h2>
      <div className="mt-12 grid w-full grid-cols-1 gap-6 md:grid-cols-3">
        {empty.steps.map((step, i) => (
          <div key={step.title} className="flex flex-col items-center text-center">
            ...
            <span className="mt-5 rounded-lg bg-surface-4 px-2.5 py-1 text-sm font-medium">Step {i + 1}</span>
            <h3 className="headline mt-3 text-[26px]">{step.title}</h3>
            <p className="mt-2 max-w-xs text-[15px] leading-relaxed text-fg-3">{step.body}</p>
          </div>
        ))}
      </div>
      {notes && notes.length > 0 && (
        <div className="mt-12 w-full max-w-3xl rounded-card border border-line bg-surface-2 p-5">
          <p className="mb-2 text-sm font-semibold">Model notes</p>
...
function StepInput() {
  return (
    <div className="absolute inset-x-5 top-8 bottom-0 rounded-t-2xl border border-line bg-surface-3 p-3">
      <p className="mb-3 text-center text-xs text-fg-2">Upload a reference</p>
      <div className="grid grid-cols-2 gap-2">
        {[
          { icon: Video, t: "Upload Video", s: "Reference clip" },
          { icon: Plus, t: "Add Elements", s: "Up to four images" },
        ].map(({ icon: Icon, t, s }) => (
          <div key={t} className="flex h-36 flex-col items-center justify-center gap-2 rounded-xl bg-surface-4">
            ...
            <p className="text-[10px] text-fg-3">{s}</p>
          </div>
        ))}
      </div>
    </div>
...
function StepPrompt() {
  return (
    <div className="absolute inset-0 p-7 text-left text-[27px] leading-snug font-medium text-fg-4">
      A woman kneeling in darkness, illuminated by a <span className="text-fg-2">warm, radiant beam</span> of light
      emerging from her raised hand.
