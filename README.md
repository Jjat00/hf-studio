...
**Audio (ElevenLabs).** La página *Audio* (`/audio`) y las herramientas MCP `text_to_speech`, `sound_effect`, `compose_music`, `isolate_voice` y `elevenlabs_account` generan voces desde texto (v3 expresivo, Multilingual v2 o Flash), efectos de sonido, música y voz aislada de un video o audio. Como todo lo de pago, primero se cotiza con la tarifa de la API (texto a voz 0,05–0,10 USD por 1.000 caracteres; efectos 0,12 USD/min; música 0,15 USD/min; aislamiento 0,12 USD/min) y se lanza con esa cotización de un solo uso; el MP3 queda en la biblioteca.
